...
public class InfoManager2 {
	...
	// 목록보기 - 5명 목록을 뒤로 붙여서 나오게끔 String 사용
	String printList() {
		String res = "";
		for (int i = 0; i < infos.length; i++) {
			if(infos[i] != null) { // 정보가 존재하면
				res += infos[i].getId() +"\t" + infos[i].getName() +"\t"+ infos[i].getAge() +"\n";
			}
		}return res;
	}
	...
	void editInfo(int id, String name) { // 이름 수정
		if (infos[id] != null) {
